...
function metadataPacks(metadata: Json | null): OrderPack[] {
  ...
  return packs.flatMap((pack) => {
    ...
    return [{ school_slug: typeof schoolSlug === "string" ? schoolSlug : undefined }];
  });
}
...
function matchingPackCount(order: ProfileOrder, school: SchoolRow): number {
  ...
  const directSlugMatch = order.school_slug?.toLowerCase() === school.slug.toLowerCase();
  const directNameMatch = order.school_name.trim().toLowerCase() === school.name.trim().toLowerCase();
  return directSlugMatch || directNameMatch ? 1 : 0;
}

export async function getSchoolProfile(idOrSlug: string): Promise<SchoolProfileData | null> {
  ...
  const [packsResult, directOrdersResult, namedOrdersResult, multiPackOrdersResult] =
    await Promise.all([
      admin
  ...
  for (const result of [directOrdersResult, namedOrdersResult, multiPackOrdersResult]) {
    if (result.error) console.error("[school-profile] orders failed:", result.error);
  }
  ...
  const paidOrders = orders.filter((order) => BOUGHT_STATUSES.has(order.status));
