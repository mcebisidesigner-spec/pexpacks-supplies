...
function johannesburgDayStart() {
  ...
  return new Date(`${value.year}-${value.month}-${value.day}T00:00:00+02:00`).toISOString();
}

const readNotificationCounts = unstable_cache(
  async (): Promise<NotificationCounts> => {
    const admin = createSupabaseAdminClient();
    const [today, pendingPayments, failedPayments, fulfilment, schools] = await Promise.all([
      admin.from("orders").select("id", { count: "exact", head: true }).gte("created_at", johannesburgDayStart()),
      admin.from("orders").select("id", { count: "exact", head: true }).in("status", ["pending_payment", "pending", "layby_active"]),
      admin.from("orders").select("id", { count: "exact", head: true }).eq("status", "payment_failed"),
      admin.from("orders").select("id", { count: "exact", head: true }).in("status", ["paid", "packing"]),
      admin.from("schools").select("id", { count: "exact", head: true }).eq("status", "pending"),
    ]);

    const failures = [today, pendingPayments, failedPayments, fulfilment, schools]
      .map((result) => result.error?.message)
      .filter(Boolean);
    if (failures.length) console.error("[admin-notifications] count query failed:", failures);
  ...
  return NextResponse.json(
    {
      ...
      pending_payments: canViewPayments || canViewOrders ? counts.pending_payments : 0,
      failed_payments: canViewPayments || canViewOrders ? counts.failed_payments : 0,
